...
abstract contract AStrategyWithdrawMinimizeTrading is Ownable, ReentrancyGuard, IStrategy {
        ...
        returns (uint256[2] memory)
    {
        ...
        IPair lpToken = IPair(IFactory(factory).getPair(token0, token1));
        {
            ...
            lpToken.approve(router, uint256(0));
            lpToken.approve(router, uint256(-1));
    ...
    function _swapTokensForExactTokens(uint256 amount, address token0, address token1) internal {
        ...
        IRouter(router).swapTokensForExactTokens(amount, path[0].myBalance(), path, address(this), now);
    }
    ...
    function _swapExactTokensForTokens(uint256 amount, address token0, address token1) internal {
        ...
        IRouter(router).swapExactTokensForTokens(amount, 0, path, address(this), now);
    }
